...
class User {
  ...
  public function __construct(\Core\Context $context, $user,$display,$pass,$email) {
    $this->m_context = $context;
    //Used for display only
    $this->displayname = $display;

    //Sanitize
    $this->clean_email = Util::sanitize($email);
    $this->clean_password = trim($pass);
    $this->username = Util::sanitize($user);

    if(Util::usernameExists($this->m_context, $this->username)) {
      $this->username_taken = true;
    } else if(Util::displayNameExists($this->m_context, $this->displayname)) {
      $this->displayname_taken = true;
    } else if(Util::emailExists($this->m_context, $this->clean_email)) {
  ...
  public function addUser() {
    ...
    if($this->status) {
      ...
      $secure_pass = Util::generateHash($this->clean_password);
      ...
      if(\Core\Context::EMAIL_ACTIVATION == "true") {
        ...
        $this->success = Util::lang("ACCOUNT_REGISTRATION_COMPLETE_TYPE2");
      } else {
        //Instant account activation
        $this->user_active = 1;
        $this->success = Util::lang("ACCOUNT_REGISTRATION_COMPLETE_TYPE1");
      }
